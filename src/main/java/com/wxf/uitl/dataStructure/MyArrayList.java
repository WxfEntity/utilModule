...
public class MyArrayList<T> implements Serializable ,IList<T>,Iterable<T> {
    ...
    private class Itr implements Iterator<T> {
         ...
         * 使用迭代器的方法移除元素
         * 和ArrayList的方法一样
         * 只能保证在单线程情况下不出现ConcurrentModificationException
         */
        public void remove() {
            if (lastRet < 0)
    ...
    /**
     * 可以前移指向的迭代器-ForwardItr
     */
    private class ForwardItr extends Itr implements ListIterator<T> {
        ForwardItr(int index) {
            super();
            cursor = index;
        }
