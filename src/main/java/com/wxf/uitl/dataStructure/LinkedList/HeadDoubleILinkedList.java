...
import com.wxf.uitl.dataStructure.Tree.Tree.DNode;

/**
 * Created by wxf on 2018/1/18.
 * 双链表的实现,带头结点(不带数据)的双链表,为了更高的效率该类包含指向尾部的指针stern
 */
public class HeadDoubleILinkedList<T> implements ILinkedList<T> {
    protected DNode<T> head; //不带数据的头结点
    protected DNode<T> stern; //指向尾部的指针

    public HeadDoubleILinkedList(){
        this.head =this.stern = new DNode<>();          //初始化头结点
    }
    ...
    public HeadDoubleILinkedList(T[] array)
    {
        ...
        if (array!=null && array.length>0)
        {
            ...
            stern =this.head.next;
            stern.prev=this.head;
            int i=1;
            while (i<array.length)
            {
                stern.next=new DNode<T>(array[i++]);
                stern.next.prev= stern;
                stern = stern.next;
            }
        }
    }
    ...
    public boolean add(T data) {
        if (data==null)
            return false;
        //创建新结点,并把其前继指针指向tail
        DNode<T> q = new DNode<T>(data, stern, null);
        stern.next=q;
        //更新尾部结点
        this.stern =q;
        return true;
